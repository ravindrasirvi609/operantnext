...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/students/studentsList");
  ...
  const onGetExporProduct = async (title?: string, worksheetname?: string) => {
    try {
      setLoading(true);
      const response = await fetch("/api/students/studentsList");
  ...
  return (
    <main className="flex flex-col items-center gap-8 p-8 md:p-24 bg-[#111827] min-h-screen">
      ...
      <button
        onClick={() => onGetExporProduct("Students", "ProductExport")}
